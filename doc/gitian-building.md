...
*Setup instructions for a Gitian build of Kiirocoin Core using a Debian VM or physical system.*
...
- [Building Kiirocoin Core](#building-bitcoin-core)
...
in the Kiirocoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Kiirocoin Core
----------------

To build Kiirocoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/snguyen-vn/kiirocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kiirocoin=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kiirocoin=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kiirocoin=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
