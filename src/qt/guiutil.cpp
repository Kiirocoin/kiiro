...
static QString kiirocoinTheme = "kiirocoinTheme";
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kiirocoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no kiirocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("kiirocoin"))
        return false;
...
{
    // Convert kiirocoin:// to kiirocoin:
    //
    //    Cannot handle this later, because kiirocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kiirocoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "kiirocoin:");
    }
...
{
    QString ret = QString("kiirocoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kiirocoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kiirocoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Kiirocoin (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kiirocoin.desktop";
    return GetAutostartDir() / strprintf("kiirocoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a kiirocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kiirocoin\n";
        else
            optionFile << strprintf("Name=Kiirocoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Kiirocoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Kiirocoin app to startup item list
...
void loadTheme()
{
    ...
    QString fileName = stylesheetDirectory + "/" + kiirocoinTheme;
