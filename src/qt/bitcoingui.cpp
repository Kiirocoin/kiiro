...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setStatusTip(tr("Send coins to a Kiirocoin address"));
	...
	receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kiirocoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kiirocoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kiirocoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a kiirocoin: URI or payment request"));

    showHelpMessageAction = new QAction(tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kiirocoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Kiirocoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Kiirocoin"); // default title
...
void BitcoinGUI::setElysiumPendingStatus(bool pending)
{
    if (!pending) {
        ...
        labelElysiumPendingIcon->setPixmap(platformStyle->SingleColorIcon(":/icons/elysium_hourglass").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
