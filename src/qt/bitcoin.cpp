...
{
    return QCoreApplication::translate("kiirocoin-core", psz).toStdString();
}
...
public:
    ...
    // migrate settings to kiirocoin. Returns true if there was migration
    ...
    // migrate directories to kiirocoin if needed
    void migrateToKiirocoin();
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // kiirocoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Kiirocoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
void BitcoinApplication::migrateToKiirocoin()
{
    migrateSettings("Zcoin", "Kiirocoin", "Zcoin-Qt", "Kiirocoin-Qt");
    migrateSettings("Zcoin", "Kiirocoin", "Zcoin-Qt-testnet", "Kiirocoin-Qt-testnet");
    ...
    boost::filesystem::path kiirocoinDefaultDataDir = GetDefaultDataDirForCoinName("kiirocoin");

    if (dataDir != zcoinDefaultDataDir)
        return;

    boost::filesystem::path dontMigrateFilePath = dataDir / ".dontmigratetokiirocoin";
    if (boost::filesystem::exists(dontMigrateFilePath) && !GetBoolArg("-migratetokiirocoin", false))
        return;
    ...
    QTextStream(&messageText) <<
        "Migrate directory structure from zcoin to kiirocoin? "
        "Directory " << GUIUtil::boostPathToQString(zcoinDefaultDataDir) <<
          " will be renamed to " << GUIUtil::boostPathToQString(kiirocoinDefaultDataDir) <<
          " and file zcoin.conf in it will be renamed to kiirocoin.conf";
    ...
    if (messageBox.exec() == QMessageBox::Yes) {
        if (RenameDirectoriesFromZcoinToKiirocoin()) {
            // Update path in settings if not set to non-default value
            setDataDirInSettings("Kiirocoin", "Kiirocoin-Qt", GUIUtil::boostPathToQString(kiirocoinDefaultDataDir));
            setDataDirInSettings("Kiirocoin", "Kiirocoin-Qt-testnet", GUIUtil::boostPathToQString(kiirocoinDefaultDataDir));
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    app.migrateToKiirocoin();

    /// 6. Determine availability of data directory and parse kiirocoin.conf
    ...
    // kiirocoin: links repeatedly have their payment requests routed to this process:
