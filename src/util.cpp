...
bool fNoDebug = false; //A temporary fix for https://github.com/kiirocoinorg/kiirocoin/issues/1011
...
{
    //A temporary fix for https://github.com/kiirocoinorg/kiirocoin/issues/1011
...
boost::filesystem::path GetDefaultDataDirForCoinName(const std::string &coinName)
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\kiirocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\kiirocoin
    // Mac: ~/Library/Application Support/kiirocoin
    // Unix: ~/.kiirocoin
#ifdef WIN32
    // Windows
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    fs::path kiirocoinDefaultDir = GetDefaultDataDirForCoinName("kiirocoin");
    if (!fs::is_directory(kiirocoinDefaultDir)) {
        // try "zcoin" in case we're upgrading from pre-kiirocoin version
        fs::path zcoinDefaultDir = GetDefaultDataDirForCoinName("zcoin");
        if (fs::is_directory(zcoinDefaultDir))
            return zcoinDefaultDir;
    }

    return kiirocoinDefaultDir;
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No kiirocoin.conf file is OK
...
bool RenameDirectoriesFromZcoinToKiirocoin()
{
    namespace fs = boost::filesystem;

    fs::path zcoinPath = GetDefaultDataDirForCoinName("zcoin");
    fs::path kiirocoinPath = GetDefaultDataDirForCoinName("kiirocoin");

    // rename is possible only if zcoin directory exists and kiirocoin doesn't
    if (fs::exists(kiirocoinPath) || !fs::is_directory(zcoinPath))
        return false;

    fs::path zcoinConfFileName = zcoinPath / "zcoin.conf";
    fs::path kiirocoinConfFileName = zcoinPath / "kiirocoin.conf";
    if (fs::exists(kiirocoinConfFileName))
        return false;

    try {
        if (fs::is_regular_file(zcoinConfFileName))
            fs::rename(zcoinConfFileName, kiirocoinConfFileName);

        try {
            fs::rename(zcoinPath, kiirocoinPath);
        }
        catch (const fs::filesystem_error &) {
            // rename config file back
            fs::rename(kiirocoinConfFileName, zcoinConfFileName);
            throw;
        }
    }
    catch (const fs::filesystem_error &) {
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Kiirocoin Core copyright is not removed by accident
    if (copyright_devs.find(_("Kiirocoin Core")) == std::string::npos) {
        strCopyrightHolders += '\n' + strPrefix + "The Kiirocoin Core developers";
    }
