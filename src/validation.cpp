...
#if defined(NDEBUG)
# error "Kiirocoin cannot be compiled without assertions."
#endif
...
static const std::vector<std::pair<uint256, uint32_t>> FROZEN_UTXOS = {
    { uint256S("7369162c6b114194ff1d6a9c769cbd5d5a68cd22650a91a9452fd7525a302018"), 3 },
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
                              ...
                              bool isCheckWalletTransaction, bool markKiirocoinSpendTransactionSerial)
{
    ...
    if (tx.IsSigmaSpend()) {
        if(markKiirocoinSpendTransactionSerial)
                        ...
                        bool isCheckWalletTransaction, bool markKiirocoinSpendTransactionSerial)
{
    ...
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache, isCheckWalletTransaction, markKiirocoinSpendTransactionSerial);
    if (!res) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        ...
                        bool isCheckWalletTransaction, bool markKiirocoinSpendTransactionSerial)
{
    return AcceptToMemoryPoolWithTime(pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markKiirocoinSpendTransactionSerial);
}


bool AcceptToMemoryPool(CTxPoolAggregate& poolAggregate, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee, bool isCheckWalletTransaction, bool markKiirocoinSpendTransactionSerial) {
    bool res = AcceptToMemoryPool(mempool, state, tx, fLimitFree, pfMissingInputs, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, isCheckWalletTransaction, markKiirocoinSpendTransactionSerial);
...
CAmount GetMasternodePayment(int nHeight, int nTime, CAmount blockValue)
{
    ...
    if (nTime >= params.stage3StartTime) {
        CMasternodeCollaterals collaterals = params.nCollaterals;
        return blockValue*collaterals.getRewardPercentage(nHeight)/100;
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            if(!txin.scriptSig.IsSigmaSpend()) {
                return state.DoS(
                    100, false,
                    REJECT_MALFORMED,
                    "CheckSpendKiirocoinTransaction: can't mix zerocoin spend input with regular ones");
            }
...
void ThreadScriptCheck() {
    RenameThread("kiirocoin-scriptch");
        ...
        for (const auto& it : block.vtx) {
            ...
            if (!tx.IsCoinBase()) {
                AcceptToMemoryPool(
                    ...
                    false /* markKiirocoinSpendTransactionSerial */
                );
            }
...
//    if (nHeight > nMNPIBlock + (nMNPIPeriod * 9)) ret += blockValue / 40; // 313520 - 50.0% - 2015-08-03
    ...
    CAmount ret = 15 * coin; //15 or 7.5 KIIRO
    ...
    if (nHeight >= consensusParams.nSubsidyHalvingFirst) {
        if (nHeight < consensusParams.nSubsidyHalvingSecond) {
            if (block.nTime >= consensusParams.stage3StartTime) {
                ...
                CAmount blockReward = GetBlockSubsidy(nHeight, consensusParams, block.nTime);

                FundPayment develpmentFundPayment = consensusParams.nDevelopmentFundPayment;
                CAmount develpmentFundReward = develpmentFundPayment.getFundPaymentAmount(nHeight, blockReward);
                int founderStartHeight = develpmentFundPayment.getStartBlock();
                if(nHeight > founderStartHeight && develpmentFundReward && !develpmentFundPayment.IsBlockPayeeValid(tx, nHeight, blockReward)) {
                    return state.Invalid(false, state.GetRejectCode(), state.GetRejectReason(), "Stage 3 Developer reward check failed");
                }

                FundPayment communityFundPayment = consensusParams.nCommunityFundPayment;
                CAmount communityFundReward = communityFundPayment.getFundPaymentAmount(nHeight, blockReward);
                int communityStartHeight = communityFundPayment.getStartBlock();
                if(nHeight > communityStartHeight && communityFundReward && !communityFundPayment.IsBlockPayeeValid(tx, nHeight, blockReward)) {
                    return state.Invalid(false, state.GetRejectCode(), state.GetRejectReason(), "Stage 3 Community reward check failed");
                }
            }
            else {
