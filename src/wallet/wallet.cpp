...
CPubKey CWallet::GetKeyFromKeypath(uint32_t nChange, uint32_t nChild, CKey& secret) {
    ...
    uint32_t nIndex = Params().GetConsensus().IsMain() ? BIP44_KIIRO_INDEX : BIP44_TEST_INDEX;
...
CBlockIndex* CWallet::ScanForWalletTransactions(CBlockIndex *pindexStart, bool fUpdate, bool fRecoverMnemonic)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        // if you are recovering wallet with mnemonics start rescan from block when mnemonics implemented in Kiirocoin
        if (fRecoverMnemonic) {
        ...
        const CCoinControl *coinControl) const
{
    ...
    if (!MoneyRange(required)) {
        throw std::invalid_argument("Request to spend more than 21 MLN kiirocoins.\n");
    }
