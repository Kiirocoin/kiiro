...
def activate_sigma_spend(denoms, spendsize, exp_spends, exp_remints, kiirocoind):
    for denom in denoms:
        count, size = denom
        for i in range(count):
            kiirocoind.nodes[0].mint(size)
    kiirocoind.nodes[0].generate(6)

    myaddr = kiirocoind.nodes[0].listreceivedbyaddress(0, True)[0]['address']
    args = {myaddr: spendsize}
    txid = kiirocoind.nodes[0].spendmany("", args)
    kiirocoind.nodes[0].generate(2)

    # Should be checked spends
    spends = kiirocoind.nodes[0].listsigmaspends(0)
